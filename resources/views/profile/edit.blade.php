@extends('layouts.app', ['activePage' => 'profile', 'titlePage' => __('Configuración de Usuario')])
                    ...
                    <form method="post" action="{{ route('profile.update') }}" autocomplete="off" class="form-horizontal">
                        ...
                        <div class="card ">
                            ...
                            <div class="card-body ">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group">
                                            <input id="fecha_nacimiento" name="fecha_nacimiento"
                                                @if (isset($vistaUsersSpecific->id_users_specific)) value="{{ date('d/m/Y', strtotime($vistaUsersSpecific->fecha_nacimiento)) }}"
                                                @else
                                                value="" @endif
                                                required />
                                            <script>
                                ...
                                <div class="row">
                                    <label class="col-sm-2 col-form-label">{{ __('Pais') }}</label>
                                    <div class="col-sm-7">
                                        <div class="form-group">
                                            <select class="form-control" data-live-search="false" id="pais" name="pais"
                                                required>
                                                <option value="">Seleccione una opción</option>
                                                @foreach ($paises as $pais)
                                                    <option value="{{ $pais->id }}"
                                                        @if (isset($vistaUsersSpecific->id_users_specific)) @if ($pais->id == $vistaUsersSpecific->id_pais) selected @endif
                                                        @endif>
                                                        {{ $pais->nombre_pais }}</option>
                                                @endforeach
                                            </select>
                                            <input type="hidden" name="id_user_specific" id="id_user_specific"
                                                @if (isset($vistaUsersSpecific->id_users_specific)) value="{{ $vistaUsersSpecific->id_users_specific }}"
                                                @else
                                                value="" @endif>
                                        </div>
                                    </div>
